...
| 5 | Wallet Intent RO | `modules/wallet.tsx` | `wallet/intents`, `wallet/provider-ops` |
...
- Wire deep-link tab `props` across players → sessions → spins (Kimi-style).
- Expand announcement i18n content fields if product needs per-locale payloads.
- Add Playwright smoke for `/admin` login + nav if browser CI is available.
