...
/** R1-M9 — Session query: list + detail + revoke + deep-links. */
...
export default function SessionsModule({ initialProps }: { initialProps?: Record<string, string> }) {
  ...
  }, [api, page, search, status, playerId]);
  ...
  return (
    <div>
      ...
      <div className="ab-toolbar">
        ...
        <input
          className="ab-input"
          placeholder={t("sessions.playerId")}
          value={playerId}
          onChange={(event) => {
          ...
          <div className="ab-modal wide" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="ab-toolbar" style={{ marginTop: 12 }}>
              <button
                ...
                onClick={() =>
                  openTab({
                    key: `players:${String(detail.session.player_id)}`,
                    titleKey: "nav.players",
                    props: { playerId: String(detail.session.player_id) },
                  })
                }
              >
                {t("sessions.openPlayer")}
              </button>
              <button
                className="ab-btn"
                onClick={() =>
                  openTab({
                    key: `spins:session:${String(detail.session.id)}`,
                    titleKey: "nav.spins",
                    props: { sessionId: String(detail.session.id) },
                  })
                }
              >
