...
import { assertLocalesComplete } from "./admin/admin-content-i18n.ts";
...
export async function listPlayerAnnouncements(db: AppDb): Promise<PlayerAnnouncement[]> {
  ...
  for (const row of rows) {
    ...
    // Defense in depth: never serve incomplete trilingual content to players.
    if (!assertLocalesComplete(parsed.locales).ok) continue;
    out.push({
      id: String(row.id),
