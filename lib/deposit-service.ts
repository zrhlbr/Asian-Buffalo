...
export async function getDepositConfig(db: AppDb): Promise<DepositConfig> {
  ...
  const parsed = parseJson<Partial<DepositConfig>>(rows[0]?.value_json, {});
  return {
...
export async function listPaymentChannels(
  ...
  const rows = await db.all<Record<string, unknown>>(sql`
    SELECT code, title_json, currency, enabled, sort_order
...
export async function upsertDepositConfig(
  ...
  await db.run(sql`
    ...
    VALUES ('deposit', ${JSON.stringify({ ...next, note: "BR-005 admin-configured" })}, ${nowSql()})
