...
import { configIsProductionReady } from "./payment-readiness.ts";
...
export async function getWithdrawalConfig(db: AppDb): Promise<WithdrawalConfig> {
  ...
  const parsed = parseJson<Partial<WithdrawalConfig> & Record<string, unknown>>(
    rows[0]?.value_json,
    {},
  );
  return {
...
export async function upsertWithdrawalConfig(
  ...
  await db.run(sql`
    ...
    VALUES (
      'withdrawal',
      ${JSON.stringify({ ...next, note: "BR-006 admin-configured" })},
      ${nowSql()}
    )
